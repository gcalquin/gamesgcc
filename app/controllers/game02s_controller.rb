class Game02sController < ApplicationController
  before_action :set_game02, only: %i[ show edit update destroy ]

  # GET /game02s or /game02s.json
  def index
    @game02s = Game02.all
  end

  # GET /game02s/1 or /game02s/1.json
  def show
  end
  ...
  # PATCH/PUT /game02s/1 or /game02s/1.json
  def update
    respond_to do |format|
  ...
  # DELETE /game02s/1 or /game02s/1.json
  def destroy
    @game02.destroy
    respond_to do |format|
